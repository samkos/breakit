...
SCHEDULER = {
             "neser": ("loadleveler",8," -l nodes=1:ppn=4,walltime=12:00:00"),
             "localhost":("unix",10,"")}
TMPDIR = "/tmp"
PYTHON_EXE = "python"
...
def get_machine():
    global CORE_PER_NODE_REGARDING_QUEUE, NODES_FAILED, MAIL_COMMAND,SCHED_TYPE
    ...
    if (machine[:4]=="fen3" or machine[:4]=="fen4" or machine[:2]=="cn"):
        ...
        tmp_directory = "/scratch/tmp/"
        CORE_PER_NODE_REGARDING_QUEUE["highmem"] = 4
        MAIL_COMMAND = """ssh cdl1.hpc.kaust.edu.sa 'mail -s \\"%s\\" %s < %s' """
        DEFAULT_QUEUE="workq"
        SUBMIT_COMMAND = 'sbatch'
...
def greps(motif, file_name, col = None, nb_lines = -1):
    """
